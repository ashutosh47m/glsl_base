...
uniform vec3		u_step_size;	//ray step size 
...
{ 
	...
	vec3 geomDir = normalize((vUV-vec3(0.5)) - u_camPos); 

	//multiply the raymarching direction with the step size to get the
	//sub-step size we need to take at each raymarching step
	vec3 dirStep = geomDir * u_step_size; 
	...
	for (int i = 0; i < MAX_SAMPLES; i++) 
	{
		...
		float sampleX = texture(u_volume, dataPos).r;	
